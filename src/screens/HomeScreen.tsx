          ...
          <FrameCard title="資產儀表板">
            <View style={styles.dashboardTop}>
              <View style={styles.dashboardSummary}>
                ...
                <Text style={[styles.heroDelta,{color:portfolio.totalPnl>=0?colors.gain:colors.loss}]}>{valuationComplete?'含息總損益 NT$ '+money(portfolio.totalPnl):'部分持股缺少核實成交價；帳務紀錄不受影響'}</Text>
              </View>
            </View>
            <View style={styles.metricRow}>
              {dashboardMetrics.map(key=>{const item=dashboardMetricInfo[key];return <MetricTile key={key} label={item.label} value={!valuationComplete&&['totalMarketValue','totalPnl','totalUnrealizedProfit'].includes(key)?'待核對':key==='holdingCount'?String(item.value):money(item.value)} caption={item.caption} {...(item.tone?{tone:item.tone}:{})}/>;})}
            </View>
          </FrameCard>
        },
