...
export type AiNewsItem=Readonly<{id:string;symbol:string;name:string;title:string;source:string;publishedAt:string;url:string;summary:string}>;
...
const summary=(title:string,symbol:string,name:string)=>{
  const clean=title.replace(/\s+-\s+[^-]+$/,'').trim();
  const focus=clean.length>88?clean.slice(0,88)+'…':clean;
  return `${symbol} ${name}：${focus}`;
};
function parseRss(xml:string,h:TrackedHolding):AiNewsItem[]{
  ...
  return blocks.slice(0,5).map((block,index)=>{
    ...
    return {id:`${h.symbol}-${publishedAt||index}-${title}`,symbol:h.symbol,name:h.name,title,source:sourceFromTitle(title),publishedAt,url,summary:summary(title,h.symbol,h.name)};
  }).filter(x=>x.title);
}
...
export function AiNewsRuntimeProvider({children}:PropsWithChildren){
  ...
  useEffect(()=>{let alive=true;AsyncStorage.getItem(KEY).then(raw=>{if(!alive||!raw)return;const p=JSON.parse(raw) as {items?:AiNewsItem[];lastUpdatedAt?:number};if(Array.isArray(p.items))setItems(p.items);if(Number.isFinite(Number(p.lastUpdatedAt)))setLastUpdatedAt(Number(p.lastUpdatedAt));}).catch(()=>{}).finally(()=>{if(alive)setHydrated(true);});return()=>{alive=false;};},[]);
