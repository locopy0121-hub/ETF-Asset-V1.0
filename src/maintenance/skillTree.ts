...
export const ENGINEER_SKILLS:readonly EngineerSkill[]=[
  ...
  {id:'frames',label:'專業框架工程',description:'背景／邊框／陰影／光圈／內外距／動畫與適配；僅對目前框架生效',tools:[
    ...
    ready('fx-img-fit','① 背景圖片填滿方式與焦點','framefx:imageFit','填滿裁切／適合完整顯示／拉伸；cover 模式可設定水平／垂直焦點及回中央，實際來源尺寸計算'),
    ...
    later('fx-background-image','① 跨手機自動嵌入背景圖片','本機背景裁切焦點已接入；content URI 圖像跨裝置持久嵌入與安全移轉仍待原生適配'),
    ready('fx-multi-gradient','① 任意角度漸層','framefx:gradientAngle','0～359° 可手動輸入或 ±1；原生 View 對角漸層；可恢復舊水平／垂直；僅當前 A 的局部外觀'),
    ready('fx-border-grad','② 雙層／厚度漸層邊框','framefx:borderGradientEnabled','真實 FrameCard 外邊緣雙層或八階深度漸層原生輪廓；雙色獨立 Color Picker／損益色、厚度與透明度'),
  ...
  {id:'animations',label:'動態動畫',description:'跑馬燈、呼吸燈、提醒動畫',tools:[ready('marquee','框架標題原生跑馬燈','framefx:titleMarqueeEnabled','僅標題溢出時水平循環；速度／間隔可編輯，降低動態時靜態省略；內容數值不動'),ready('blink','閃爍','framefx:blinkEnabled','真實框架原生邊框閃爍；獨立系統調色盤／損益色與週期；降低動態時靜態顯示；不遮蔽數據')]},
