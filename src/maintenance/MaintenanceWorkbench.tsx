...
export function MaintenanceWorkbench(){
  ...
  return <View style={[styles.dock,{backgroundColor:theme.palette.surface,borderTopColor:theme.palette.primary}]}>
    <View style={styles.head}>
      <View style={{flex:1}}>
        <Text style={[styles.headline,{color:theme.palette.text}]}>🔧 駐點維護工程師 · {session.title}</Text>
        <Text style={{color:theme.palette.textSecondary,fontSize:11}}>A 已指定：{session.scope==='frame'?'框架':'元件'}｜B 技能 → C 工具 → D 細節</Text>
      </View>
    ...
    <ScrollView style={styles.scroller} nestedScrollEnabled keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator>
      <Text style={[styles.hint,{color:theme.palette.textSecondary}]}>全部技能均可查看；未接入當前元件的工具會明確標示，不提供虛假效果。上方原畫面即時呈現暫存修改。</Text>
        ...
        {openSkill===skillItem.id?<View style={styles.toolList}>
          ...
          {skillItem.tools.map(tool=><View key={tool.id} style={{marginTop:8}}>
            <Pressable accessibilityRole="button" onPress={()=>setOpenTool(current=>current===tool.id?null:tool.id)} style={styles.toolRow}>
              ...
              <Text style={{color:tool.status==='ready'?theme.palette.primary:theme.palette.textSecondary,fontSize:11}}>{tool.status==='ready'?'細節 ›':'待接入 ›'}</Text>
            </Pressable>
            {openTool===tool.id?<View style={[styles.detail,{backgroundColor:theme.palette.surfaceMuted}]}>
              ...
              <ToolDetails tool={tool} instance={instance}/>
            </View>:null}
          </View>)}
        </View>:null}
      </View>)}
