...
接續中央既有待接線技能「跑馬燈」：A 當前真實 FrameCard → B 特效與動態 → C 框架標題原生水平跑馬燈。預設 OFF，速度 24–180 dp/s、標題間距 12–80 dp；標題超出容器才以 native-driver 循環。原框架操作按鈕與財務子內容維持不動；降低動態時顯示單行省略號，並即時監聽開關變化。原位草稿預覽、取消還原、正式套用儲存於 FrameEffects；隨既有 SAF 備份。

唯一中央 184 項維持不變，源碼宣告已接線 **152／待適配 32**；僅 FrameCard 標題適配，子元件動畫仍須按原生種類逐項補齊。源碼／CI／APK／實機測試分別回報，手機溢出、操作按鈕排版與長時間動畫仍待實機。帳務核心、已固化費稅、行情來源與既有備份皆不更動；開發紀錄不預先宣告本版 APK PASS。
