...
for(const id of ['ai:prompt-title','ai:quick-action:','ai:conversation','ai:composer'])
  assert.ok(ai.includes(id),'V3.0.6 native AI target lost: '+id);
const pkg=JSON.parse(read('package.json'));assert.equal(pkg.version,'3.0.6','layout repair is a V3.0.6 QA revision, NOT a V3.0.6 feature');
console.log('V3.0.6 layout regression gate: stable ON/OFF metric grid, overlay-only selection, no finance rewrite and AI preservation PASS');
