...
assert.equal(audit.pending,38,'three prior skills plus two new V3.0.19 tools leave 39 declared pending');
assert.equal(audit.readyDeclared,146);
...
assert.equal(all.filter(tool=>tool.id.startsWith('advanced-')&&tool.status==='ready').length,6);
...
assert.equal(pkg.version,'3.0.19');assert.equal(app.expo.version,'3.0.19');
