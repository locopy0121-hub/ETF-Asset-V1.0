...
// V3.0.8: one shared skill tree, isolated per-card caption/title styles and native-only font choices.
...
assert.equal(normalizeTargetOverride({prefixOffsetY:-40}).prefixOffsetY,-24);
assert.equal(normalizeTargetOverride({prefixOffsetY:500}).prefixOffsetY,24);
...
assert.ok(workbench.includes('prefixOffsetY:[-24,24,1]'));
...
assert.equal(pkg.version,'3.0.8');
assert.equal(app.expo.version,'3.0.8');
assert.equal(app.expo.android.versionCode,30008);
assert.equal(app.expo.ios.buildNumber,'30008');
...
assert.ok(read('src/settings/BackupService.ts').includes("const APP_VERSION='3.0.8'"));
...
console.log('V3.0.8 engineer text detail: scoped model, native view wiring, locked finance and version gates PASS');
