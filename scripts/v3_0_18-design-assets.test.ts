...
assert.equal(audit.readyDeclared,145,'only two additional genuinely wired skills');
assert.equal(audit.pending,39);
...
assert.ok(backup.includes("APP_VERSION='3.0.18'"));
...
console.log('V3.0.18: 5-slot design assets, visual-only sanitization, native target/frame preview, safe local sync PASS');
console.log('V3.0.18: persisted favorites/recent shortcuts, no extra AB level, backup inclusion PASS');
