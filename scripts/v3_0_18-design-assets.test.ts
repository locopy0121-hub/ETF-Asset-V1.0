...
assert.equal(audit.readyDeclared,151,'visual history is the only newly wired V3.0.25 skill');
assert.equal(audit.pending,33);
...
assert.ok(backup.includes("APP_VERSION='3.0.25'"));
...
console.log('V3.0.25: 5-slot design assets, visual-only sanitization, native target/frame preview, safe local sync PASS');
console.log('V3.0.25: persisted favorites/recent shortcuts, no extra AB level, backup inclusion PASS');
