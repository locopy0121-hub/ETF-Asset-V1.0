...
assert.equal(audit.readyDeclared,148,'visual history is the only newly wired V3.0.22 skill');
assert.equal(audit.pending,36);
...
assert.ok(backup.includes("APP_VERSION='3.0.22'"));
...
console.log('V3.0.22: 5-slot design assets, visual-only sanitization, native target/frame preview, safe local sync PASS');
console.log('V3.0.22: persisted favorites/recent shortcuts, no extra AB level, backup inclusion PASS');
