...
assert.equal(audit.readyDeclared,153,'visual history is the only newly wired V3.0.27 skill');
assert.equal(audit.pending,31);
...
assert.equal(pkg.version,'3.0.27');assert.equal(app.expo.version,pkg.version);
assert.equal(app.expo.android.versionCode,30027);assert.equal(app.expo.ios.buildNumber,'30027');
assert.ok(backup.includes("APP_VERSION='3.0.27'"));
...
console.log('V3.0.27: 5-slot design assets, visual-only sanitization, native target/frame preview, safe local sync PASS');
console.log('V3.0.27: persisted favorites/recent shortcuts, no extra AB level, backup inclusion PASS');
