...
assert.equal(audit.readyDeclared,154,'current V3.0.28 source-adapter totals must match explicit native wiring');
assert.equal(audit.pending,30);
...
assert.ok(backup.includes("APP_VERSION='3.0.28'"));
...
console.log('V3.0.28: 5-slot design assets, visual-only sanitization, native target/frame preview, safe local sync PASS');
console.log('V3.0.28: persisted favorites/recent shortcuts, no extra AB level, backup inclusion PASS');
