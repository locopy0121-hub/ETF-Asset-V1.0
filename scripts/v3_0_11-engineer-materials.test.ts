...
assert.equal(pkg.version,'3.0.19');
assert.equal(app.expo.version,'3.0.19');
assert.equal(app.expo.android.versionCode,30019);
assert.equal(app.expo.ios.buildNumber,'30019');
assert.ok(read('src/screens/SettingsScreen.tsx').includes("const VERSION='3.0.19'"));
assert.ok(read('src/screens/SettingsScreen.tsx').includes("const BUILD='30019'"));
assert.ok(read('src/settings/BackupService.ts').includes("const APP_VERSION='3.0.19'"));
