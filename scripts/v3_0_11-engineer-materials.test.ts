...
assert.equal(pkg.version,'3.0.27');
assert.equal(app.expo.version,'3.0.27');
...
assert.ok(read('src/screens/SettingsScreen.tsx').includes("const BUILD='30027'"));
assert.ok(read('src/settings/BackupService.ts').includes("const APP_VERSION='3.0.27'"));
