...
assert.ok(read('src/screens/SettingsScreen.tsx').includes("const VERSION='3.0.14'"));
assert.ok(read('src/screens/SettingsScreen.tsx').includes("const BUILD='30014'"));
assert.ok(read('src/settings/BackupService.ts').includes("const APP_VERSION='3.0.14'"));
assert.ok(read('.github/workflows/ci.yml').includes('TF-Asset-V3.0.14-QA.apk'));
...
console.log('V3.0.14 material presets, native rendering, local reset, independent alpha and protected finances: PASS');
