...
assert.ok(read('src/screens/SettingsScreen.tsx').includes("const VERSION='3.0.15'"));
assert.ok(read('src/screens/SettingsScreen.tsx').includes("const BUILD='30015'"));
assert.ok(read('src/settings/BackupService.ts').includes("const APP_VERSION='3.0.15'"));
assert.ok(read('.github/workflows/ci.yml').includes('TF-Asset-V3.0.15-QA.apk'));
...
console.log('V3.0.15 material presets, native rendering, local reset, independent alpha and protected finances: PASS');
