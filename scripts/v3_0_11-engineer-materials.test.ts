...
assert.equal(pkg.version,'3.0.11');
assert.equal(app.expo.version,'3.0.11');
assert.equal(app.expo.android.versionCode,30011);
assert.equal(app.expo.ios.buildNumber,'30011');
assert.ok(read('src/screens/SettingsScreen.tsx').includes("const VERSION='3.0.11'"));
assert.ok(read('src/screens/SettingsScreen.tsx').includes("const BUILD='30011'"));
assert.ok(read('src/settings/BackupService.ts').includes("const APP_VERSION='3.0.11'"));
assert.ok(read('.github/workflows/ci.yml').includes('TF-Asset-V3.0.11-QA.apk'));
...
console.log('V3.0.11 material presets, native rendering, local reset, independent alpha and protected finances: PASS');
