...
assert.equal(pkg.version,'3.0.26');
assert.equal(app.expo.version,'3.0.26');
assert.equal(app.expo.android.versionCode,30026);
assert.equal(app.expo.ios.buildNumber,'30026');
assert.ok(read('src/screens/SettingsScreen.tsx').includes("const VERSION='3.0.26'"));
assert.ok(read('src/screens/SettingsScreen.tsx').includes("const BUILD='30026'"));
assert.ok(read('src/settings/BackupService.ts').includes("const APP_VERSION='3.0.26'"));
