...
assert.equal(audit.readyDeclared,149);assert.equal(audit.pending,35,
  'V3.0.23 wires exactly one existing legacy history tool, not an invented new category');
...
assert.equal(resolveSkillAdapter(historyTool,{...context,scope:'instance',kind:'text',instanceOwned:true}).status,
 'active','V3.0.23 adds owned installed instance visual history without a new catalog tool');
...
assert.equal(app.expo.android.versionCode,30023);
assert.equal(app.expo.ios.buildNumber,'30023');
...
console.log('V3.0.23 A-only local visual history: sanitization/restore/capping/real adapter/backup PASS');
console.log('184 catalog / 149 wired declarations / 35 pending / locked finance and AB regression PASS');
