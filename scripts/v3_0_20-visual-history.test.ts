...
assert.equal(audit.readyDeclared,156);assert.equal(audit.pending,28,
  'V3.0.30 wires exactly one existing legacy history tool, not an invented new category');
...
assert.equal(resolveSkillAdapter(historyTool,{...context,scope:'instance',kind:'text',instanceOwned:true}).status,
 'active','V3.0.30 adds owned installed instance visual history without a new catalog tool');
...
console.log('V3.0.30 A-only local visual history: sanitization/restore/capping/real adapter/backup PASS');
