...
assert.equal(audit.readyDeclared,155);assert.equal(audit.pending,29,
  'V3.0.29 wires exactly one existing legacy history tool, not an invented new category');
...
assert.equal(resolveSkillAdapter(historyTool,{...context,scope:'instance',kind:'text',instanceOwned:true}).status,
 'active','V3.0.29 adds owned installed instance visual history without a new catalog tool');
...
assert.equal(pkg.version,'3.0.29');assert.equal(app.expo.version,'3.0.29');
...
console.log('V3.0.29 A-only local visual history: sanitization/restore/capping/real adapter/backup PASS');
