...
assert.equal(audit.readyDeclared,154);assert.equal(audit.pending,30,
  'V3.0.28 wires exactly one existing legacy history tool, not an invented new category');
...
assert.equal(resolveSkillAdapter(historyTool,{...context,scope:'instance',kind:'text',instanceOwned:true}).status,
 'active','V3.0.28 adds owned installed instance visual history without a new catalog tool');
...
assert.equal(app.expo.android.versionCode,30028);
assert.equal(app.expo.ios.buildNumber,'30028');
...
console.log('V3.0.28 A-only local visual history: sanitization/restore/capping/real adapter/backup PASS');
