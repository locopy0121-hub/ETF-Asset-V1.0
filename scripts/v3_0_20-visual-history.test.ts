...
assert.equal(audit.readyDeclared,148);assert.equal(audit.pending,36,
  'V3.0.22 wires exactly one existing legacy history tool, not an invented new category');
...
assert.equal(resolveSkillAdapter(historyTool,{...context,scope:'instance',kind:'text',instanceOwned:true}).status,
 'active','V3.0.22 adds owned installed instance visual history without a new catalog tool');
...
assert.equal(pkg.version,'3.0.22');assert.equal(app.expo.version,'3.0.22');
...
console.log('V3.0.22 A-only local visual history: sanitization/restore/capping/real adapter/backup PASS');
console.log('184 catalog / 148 wired declarations / 36 pending / locked finance and AB regression PASS');
