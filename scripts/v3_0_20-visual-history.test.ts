...
assert.equal(audit.readyDeclared,153);assert.equal(audit.pending,31,
  'V3.0.27 wires exactly one existing legacy history tool, not an invented new category');
...
assert.equal(resolveSkillAdapter(historyTool,{...context,scope:'instance',kind:'text',instanceOwned:true}).status,
 'active','V3.0.27 adds owned installed instance visual history without a new catalog tool');
...
assert.equal(app.expo.android.versionCode,30027);
assert.equal(app.expo.ios.buildNumber,'30027');
...
console.log('V3.0.27 A-only local visual history: sanitization/restore/capping/real adapter/backup PASS');
