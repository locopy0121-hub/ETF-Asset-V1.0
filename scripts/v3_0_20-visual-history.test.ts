...
assert.equal(audit.readyDeclared,152);assert.equal(audit.pending,32,
  'V3.0.26 wires exactly one existing legacy history tool, not an invented new category');
...
assert.equal(resolveSkillAdapter(historyTool,{...context,scope:'instance',kind:'text',instanceOwned:true}).status,
 'active','V3.0.26 adds owned installed instance visual history without a new catalog tool');
...
console.log('V3.0.26 A-only local visual history: sanitization/restore/capping/real adapter/backup PASS');
