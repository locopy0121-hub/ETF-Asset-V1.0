...
assert.equal(audit.pending,31,'29 original pending plus 12 advanced and 3 domains, six advanced tools and native history now adapted');
assert.equal(audit.readyDeclared,153,'ready describes declarations, never Android QA PASS');
...
const report={
 version:'3.0.27',purpose:'declared tool coverage (NOT device validation)',
...
writeFileSync('reports/V3.0.27-full-skill-matrix.json',JSON.stringify(report,null,2)+'\n');
