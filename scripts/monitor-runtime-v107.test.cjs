...
assert.match(nativeModule,/getMonitorStatus/,'native module must expose monitor status');
assert.match(nativeModule,/permissionRequired/,'native status must distinguish missing overlay permission');
