...
assert.match(native,/ACTION_FORCE_REFRESH/);assert.doesNotMatch(native,/startActivity\(launch\)/);assert.match(native,/goAsync\(\)/);assert.match(native,/widget_quote_overrides/);assert.match(native,/views\.setOnClickPendingIntent\(R\.id\.widget_root,refreshPending\)/);assert.match(native,/View\.INVISIBLE/);assert.match(native,/widget_refresh,refreshPending/);
