...
assert.equal(pkg.version,'3.0.4');assert.equal(app.expo.version,'3.0.4');
assert.equal(app.expo.android.versionCode,30004);assert.equal(app.expo.ios.buildNumber,'30004');
assert.ok(read('.github/workflows/ci.yml').includes('TF-Asset-V3.0.4-QA.apk'));
console.log('V3.0.4 measured spatial math, strict ±1, optional drag-only snap, global color flags and cross-page contracts: AUTOMATED PASS');
