...
assert.equal(pkg.version,'3.0.6');assert.equal(app.expo.version,'3.0.6');
assert.equal(app.expo.android.versionCode,30006);assert.equal(app.expo.ios.buildNumber,'30006');
assert.ok(read('.github/workflows/ci.yml').includes('TF-Asset-V3.0.6-QA.apk'));
console.log('V3.0.6 measured spatial math, strict ±1, optional drag-only snap, global color flags and cross-page contracts: AUTOMATED PASS');
