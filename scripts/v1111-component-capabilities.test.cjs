...
assert.match(modal,/const editFrame=pageKey==='portfolio'\?'holding-view':pageKey==='home'\?'holding-quotes':null/,'Only home and portfolio may receive a targeted initial editor frame');assert.match(modal,/setOpenFrame\(initialContentTab\?editFrame:\(pageKey==='portfolio'\?editFrame:null\)\)/,'Portfolio defaults to holding-view; explicit A\/B launch opens the target frame');assert.match(modal,/setOpenGroup\(initialContentTab&&editFrame\?editFrame\+':content':\(pageKey==='portfolio'\?'holding-view:content':null\)\)/,'Portfolio list controls default open; explicit A\/B launcher opens content tools');assert.match(modal,/setOpenGroup\(null\)/,'Opening another frame must collapse the previous inner group');assert.match(modal,/AccordionGroup/);
