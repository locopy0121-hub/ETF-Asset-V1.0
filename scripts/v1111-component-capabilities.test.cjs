...
assert.match(modal,/setOpenFrame\(pageKey==='portfolio'\?'holding-view':null\)/,'Portfolio editing should open holding-view by default while other pages stay collapsed');assert.match(modal,/setOpenGroup\(pageKey==='portfolio'\?'holding-view:content':null\)/,'Portfolio list controls should be visible on entry');assert.match(modal,/setOpenGroup\(null\)/,'Opening another frame must collapse the previous inner group');assert.match(modal,/AccordionGroup/);
