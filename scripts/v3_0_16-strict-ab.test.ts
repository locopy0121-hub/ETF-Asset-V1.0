...
assert.equal(pkg.version,'3.0.16');assert.equal(app.expo.version,pkg.version);
assert.equal(app.expo.android.versionCode,30016);assert.equal(app.expo.ios.buildNumber,'30016');
...
console.log('V3.0.16 AB actual A → B length/width/color → C direct size/profit/gradient: PASS');
